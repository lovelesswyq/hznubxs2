...
class unusePublishedManager(models.Manager):
    def get_queryset(self):
        return super(unusePublishedManager,
                    self).get_queryset().filter(status='published')

# Create your models here.
class Unuse(models.Model):
    STATUS_CHOICES = (
        ('draft', 'Draft'),
        ('published', 'Published'),
    )
    ...
    published = unusePublishedManager() # Our custom manager.
    
    def get_absolute_url(self):
        return reverse('unuse:unuse_detail',
...
class unuseComment(models.Model):
    post = models.ForeignKey(Unuse, related_name='comments')
